...
# 加载训练目录对应的标签，参数：label_source_file标签所在的csv文件
def load_labels(label_source_file: str):
    labels = {}
    with open(label_source_file, 'r') as obj:
...
# 加载数据，参数：train_dirs训练所用图像所在的文件夹列表，labels用load_labels返回，应该是字典
def load_data(train_dirs: list, labels):
    train_list = list(chain.from_iterable(glob.glob(os.path.join(train_dir,'*.jpg')) for train_dir in train_dirs))

    print(f"Train Data: {len(train_list)}")

    return train_list, labels

labels = load_labels('./label.csv')
train_list, labels = load_data(['./Freeform/save/', './Northwind/save/'], labels)
...
for epoch in range(epochs):
    ...
    for batch_idx, (data, label) in enumerate(tqdm(train_loader)):
        ...
        print(
            f"Train Epoch: {epoch+1} [{batch_idx * len(data)}/{len(train_loader.dataset)} ({100. * batch_idx / len(train_loader):.0f}%)]\tLoss: {loss.item():.6f}"
        )
